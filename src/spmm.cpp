...
public:
  ...
  backward(torch::autograd::AutogradContext *ctx,
    ...
    auto rowptr = saved[0], col = saved[1], values = saved[2], colptr = saved[3], row = saved[4], csr2csc = saved[5], dense = saved[6];
...
torch::Tensor spmm_sum(torch::Tensor rowptr, torch::Tensor col,
                       ...
                       torch::Tensor row, torch::Tensor csr2csc,
                       torch::Tensor dense, bool has_value, int64_t algorithm) {
  return SpMMSum::apply(rowptr, col, values, colptr, row, csr2csc, dense, has_value, algorithm);
}
